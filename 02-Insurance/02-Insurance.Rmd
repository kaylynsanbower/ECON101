...
Say your utility function is $u(w)=\sqrt{w}$, and you're starting with $w=$ <span>&#36;</span>100. I propose a lottery in which I flip a coin...heads you win <span>&#36;</span>20 and tails you lose <span>&#36;</span>20. 
...
3. Calculate this person's utility if they were to incur the expected cost of illness with certainty. Is this utility higher or lower than what you found in part (2)?
...
1. Curvature of the demand function
...
\draw (0,3) node[anchor=north east]{AC} -- (6,2);
\draw (0,3) node[anchor=south west]{MC} -- (6,0.5);
...
1. Demand above MC: full insurance and no problems from adverse selection
2. Demand intersects MC at some point: underinsurance from people willing and able but ultimately not purchasing
3. Demand below MC: death spiral! (or, less dramatic, unraveling)<br>
...
---
# Examples
Assume the insurer's cost function is $C=100q - 2q^{2}$, where $q$ denotes the number of people enrolled in the plan. Further assume that demand is given by $D=110-3q$. Assuming the insurer enters the market with $p_{1}=65$,<br>

1. what is the next year's price (if profits are <span>&#36;</span>0)? 
2. What is the equilibrium price?

---
# Answer
To solve this, we need to first calculate the marginal cost curve, $mc=100-4q$, and the average cost curve, $ac=100-2q$. At a price of $p=65$, we know that quantity demanded will be $q=15$. At that point, the average cost is then <span>&#36;</span>70. Since $AC<p$, the firm is losing money. If they set prices to break-even, then next period's price would be set to their observed AC, $p_{2}=$ <span>&#36;</span>70. We can find the equilibrium price as the point where the AC and D curves intersect, which occurs at $p=$ <span>&#36;</span>80.
