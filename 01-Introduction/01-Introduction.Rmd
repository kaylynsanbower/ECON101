...
```{r spend, echo=FALSE, warning=FALSE}
...
```{r spend-inf-mort, echo=FALSE}
oecd.dat %>% filter(year>=1970) %>% group_by(country) %>% fill(c(tot_exp, mort_inf), .direction="updown") %>%
...
```{r spend-mat-mort, echo=FALSE}
oecd.dat %>% filter(year>=1970 & year<=2012) %>% group_by(country) %>% fill(c(tot_exp, mort_mat), .direction="updown") %>%
...
(Read the full document [here](https://github.com/imccart/Econ-372/Syllabus/blob/master/Econ372-Syllabus.pdf).)
...
3. Many reasons:
  - Standard health economics issues (adverse selection, experience goods, etc.)
  - Fragmented system
